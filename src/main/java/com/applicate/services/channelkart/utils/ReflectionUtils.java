...
public class ReflectionUtils {
    ...
    public static <T> Optional<Class<? extends T>> findSubClassByName(Class<T> clazz, String name) {
        return reflections.getSubTypesOf(clazz).stream().filter(item -> item.getSimpleName().equalsIgnoreCase(name)).findFirst();
    }

    @SuppressWarnings("unchecked")
    public static Set<Class<?>> searchTypeFields(Class<?> targetClass, Class<?> searchType) {
        return getAllFields(targetClass, field -> isFieldTypeMatching(field, searchType)).stream().filter(field -> isFieldTypeMatching(field, searchType)).map(ReflectionUtils::getFieldOrCollectionGenericType).collect(Collectors.toSet());
    }
    ...
    private static Method getMethod(Object object, String methodName, Class<?>[] classes) {
        ...
        try {
            ...
            method = Arrays.stream(object.getClass().getMethods()).filter(m -> m.getName().equals(methodName)).filter(m -> matchArgument(m, classes)).findFirst().orElseThrow(() -> new ReflectionException("Could not find method:" + methodName));
        }
        return method;
    }
