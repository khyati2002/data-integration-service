...
public class SupplierMetaDataService extends AbstractCDMService<SupplierMetadata> {
		...
		if (!saveItemsList.get(0).isEmpty()) {
			getDslContext().batchInsert(
					saveItemsList.get(0).stream()
							.map(outlet -> getDslContext().newRecord(CK_SUPPLIER_METADATA, outlet))
	...
	public List<List<SupplierMetadata>> getItemsToSaveList(List<SupplierMetadata> supplierMetadataList){
		...
		List<String> outletCodes = supplierMetadataList.stream()
				.map(SupplierMetadata::getId)
				.collect(Collectors.toList());

		Map<String, com.salescode.dim.jooq.generated.tables.pojos.SupplierMetadata> savedList = getDslContext()
				.selectFrom(CK_SUPPLIER_METADATA)
				.where(CK_SUPPLIER_METADATA.ID.in(outletCodes))
				.fetch()
				.intoMap(CK_SUPPLIER_METADATA.ID, record -> record.into(com.salescode.dim.jooq.generated.tables.pojos.SupplierMetadata.class));
