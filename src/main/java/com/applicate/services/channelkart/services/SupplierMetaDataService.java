...
public class SupplierMetaDataService extends AbstractCDMService<SupplierMetadata>{
    ...
    public void saveSupplier(String userLoginId, List<SupplierMetadata> supplierMetaDataList) {
        ...
        List<SupplierMetadata> itemsToSave = supplierMetaDataList.stream()
                .filter(newItem -> !hasMatchingItem(newItem, existingSupplierMetaData))
    ...
    private boolean hasMatchingItem(SupplierMetadata newItem, List<SupplierMetadata> existingList) {
        return existingList.stream()
                .anyMatch(existingItem -> compareBusinessFields(existingItem, newItem));
    }

    private boolean compareBusinessFields(SupplierMetadata existing, SupplierMetadata newSupplier) {
        return Objects.equals(existing.getActiveStatus(), newSupplier.getActiveStatus()) &&
                Objects.equals(existing.getActiveStatusReason(), newSupplier.getActiveStatusReason()) &&
                Objects.equals(existing.getExtendedAttributes(), newSupplier.getExtendedAttributes()) &&
                Objects.equals(existing.getLob(), newSupplier.getLob()) &&
                Objects.equals(existing.getLevel(), newSupplier.getLevel()) &&
                Objects.equals(existing.getMax(), newSupplier.getMax()) &&
                Objects.equals(existing.getMin(), newSupplier.getMin()) &&
                Objects.equals(existing.getType(), newSupplier.getType()) &&
                Objects.equals(existing.getUserLoginid(), newSupplier.getUserLoginid()) &&
                Objects.equals(existing.getSource(), newSupplier.getSource());
    }

    public List<SupplierMetadata> findByUserLoginId(String userLoginId) {
        return getDslContext().selectFrom(CK_SUPPLIER_METADATA)
                .where(CK_SUPPLIER_METADATA.USER_LOGINID.eq(userLoginId))
                .fetchInto(SupplierMetadata.class);
    }
            ...
            getDslContext().batchInsert(
                    supplierMetaDataList.stream()
                            .map(supplier -> {
                                ...
                                supplier.setChanged(Boolean.TRUE);
