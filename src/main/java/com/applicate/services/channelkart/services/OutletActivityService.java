...
public class OutletActivityService extends AbstractCDMService<OutletActivity>{
	...
	private List<List<OutletActivity>> getItemsToSaveList(List<OutletActivity> entityList) {
		...
		List<String> ids = entityList.stream()
				.map(OutletActivity::getId)
				.collect(Collectors.toList());

		Map<String, OutletActivity> savedList = getDslContext().selectFrom(CK_OUTLET_ACTIVITY)
				.where(CK_OUTLET_ACTIVITY.ID.in(ids))
				.fetch()
				.intoMap(CK_OUTLET_ACTIVITY.ID, recordEntry -> recordEntry.into(OutletActivity.class));
