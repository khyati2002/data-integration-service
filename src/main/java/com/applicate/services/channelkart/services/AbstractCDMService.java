...
import com.applicate.services.channelkart.enrichments.DataEnrichmentService;
...
//        return saved;
    ...
    public List<T> batchSave(Iterable<T> iterObj, IdGenerator idGenerator) {
        ...
        if (isNativeBatchSave(iterObj)) {
            throw new RuntimeException("Native batch save not allowed");
        } else {
...
//        }
    ...
    public void delete(CommonDataModel cdm){
        if(StringUtils.isNotEmpty(cdm.getId())){
            ...
            log.warn("No id present in {} cdm object[[{}]] . Skipping deletion.",cdm.getClass().getSimpleName(),cdm);
        }
    }
