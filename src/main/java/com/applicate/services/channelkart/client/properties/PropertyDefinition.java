...
public enum PropertyDefinition {
	IGNORE_ORDER_SYNC_FOR_TEST_USER("ignore.order.sync","[]", "ignore order sync on client side for these all test loginId's"),

	BYPASS_TOKEN_EXPIRY_VALIDATION("bypass.token.expiry.validation" , Constants.FALSE,
			"If value is true then we skip the token expiry validation"),

	FETCH_PSR_FROM_QUERY("fetch.psr.from.query" , Constants.FALSE,
			"If value is true then we use the query to fetch psr"),
	...
	SAGA_PREPROCESS_WRAPPER("saga.preprocess.wrapper", Constants.FALSE, "If true, will use saga preprocess wrapper instead of original preprocess service"),
	...
	DESIGNATIONS_WITH_RESOURCE_ACCESS_PERMISSION("designations.with.resource.access.permission", "",
			"users with defined designations will get resource access permssion (example:admin, supplier)"),

	SUPPLIER_USER_ALLOWED_TO_FETCH_USERS_WITH_DESIGNATIONS("supplier.user.allowed.to.fetch.users.with.designations", "",
			"users with supplier designations will get resource access permission to fetch the users with specified designations"),
	...
	DO_TAX_CALCULATION("do.tax.calculation", Constants.TRUE, "If true then tax calculation will be performed"),
	...
	REPORT_PASSWORD_ENABLED("report.password.enabled", Constants.FALSE,
			"If true then report encryption will be enabled "),
	...
	SAGA_AUTO_TOPOLOGICAL_SORT("saga.auto.topological.sort", Constants.FALSE,
			"(Only if using SagaOrchestratorConfiguration) If true then saga will be auto executed in topological sort order, ignoring the order in which the steps are added to the orchestrator"),
	...
	USE_SUPPLIER_FROM_OUTLET_METADATA("use.supplier.from.outlet.metadata", Constants.FALSE,
			"If true then supplier information will be fetched from outlet metadata entity"),
	...
	SCHEME_UPSERT_TIME("scheme.upsert.time", "24",
			"time to decide if scheme is to be upserted or deleted->inserted"),
	...
	MDM_DATA_PUBLISH_TO_MICRO_SERVICE("mdm.data.publish.to.micro.service", Constants.TRUE,
			"If true then it will push data to micro service instead of storing into salescode database"),
	...
	FETCH_STOCK_FILTER("fetch.stock.filter", "",
			"The name of the stock filter. This is used to get the field name on the basis of which the stock will get filtered from the stock table."),
	...
	ADMIN_ALLOWED_TO_CHANGE_USER_PASSWORD("admin.allowed.to.change.password", Constants.FALSE,
			"If true, then admin will be allowed to change user passwords "),
	...
	APPEND_SALES_REP_TO_QUERY("append.sales.rep.to.query",Constants.FALSE, "If true, then sales rep will be appended to order / sale", Constants.BOOLEAN),
	...
	ALLOW_MULTIPLE_INVOICE("allow.multiple.invoice",Constants.FALSE,
			"If true, then multiple invoice can be generated for same order", Constants.BOOLEAN),
	...
	APPLY_TOV_ON_QTY("apply.tov.on.qty", Constants.FALSE, "apply tov coupon on qty", Constants.BOOLEAN),
	...
	GET_STOCK_ON_BATCH_CODE_AND_SUPPLIER("get.stock.on.batch.code.and.supplier", Constants.FALSE, "to check stock on batch code and supplier"),

	COPY_ORDER_OBJECT("copy.order.object", Constants.FALSE, "to copy the order object in order discount enrichment"),

	GET_EXCLUSION_KEYS("get.exclusion.keys", "outlet_code", "to get the column_name of the values to be considered in exclusion"),

	INFRA_CHANGE_EXCLUSIONS("infra.changes.exclusions", Constants.FALSE, "to enable infra changes for exclusions list in coupon dashboard"),

	ORIGINAL_PRICE_ORDER_DETAILS("original.price.order.details", Constants.FALSE, "to get original priced order details in case of price updating schemes"),
	BALANCE_ROLLBACK_ORDER_STATUS("balance.rollback.order.status", "0", "rollback balance on order status expired or rejected"),
	...
	CHECK_DEVICE_ID_FOR_SIGN_IN("check.device.id.for.signin", Constants.FALSE,
			"if true, we will check if the device id is associated with multiple users and fail the request"),

	NUMBER_OF_DEVICE_ID_FOR_SKIP("number.of.device.id.for.check", "1",
			"will define the number of logins allowed per device id"),

	PRODUCT_CODE_IN_SECONDARY_PRODUCTS("product.code.in.secondary.products", "skuCode",
			"skuCode/batchCode - Secondary products will be identified based on this code"),
	...
	OUTLETCACHE_CLIENT_PROPERTY("outletCacheProperties","","It will add specified outlet property to outlet Cache"),
	...
	FILTER_BY_DEFAULT_BARCODE("filter.by.default.barcode", Constants.FALSE, "If true, it will add extended attributes to validate api response"),
	...
	CREATE_DELIVERY_INFO_FOR_INVOICE("create.delivery.info.for.invoice",Constants.FALSE,"if true delivery info will be created for invoice"),
	CREATE_GRN_FOR_INVOICE("create.grn.for.invoice",Constants.FALSE,"if true grn will be created for invoice"),
	CREATE_CUSTOM_INVOICE_NUMBER("create.custom.invoice.number",Constants.FALSE,"if true custom invoice number will be created"),
	...
	IS_SHIPTO_AND_SOLDTO_ENABLED("is.shipTo.and.soldTo.enabled",Constants.FALSE,"if true outletCode will set as shipTo and loginId as soldTo"),
	USE_LOGGED_IN_USER_TO_FETCH_ORDER_PRICE("use.logged.in.user.for.order.price",Constants.FALSE,"if enabled logged in user will be used to fetch pricing for order"),
	BASIC_PRENT_INFO_ACCESS("basic.parent.info.access", Constants.FALSE, "If true, it will  allow user to get his parent info with or without location info"),
	CREATE_SAME_ORDER_DETAIL("create.same.order.detail",Constants.TRUE ,"If false, it will create a separate order detail for the free sku even if the sku was bought in the order" ),
	ADD_BALANCE_TO_EXTENDED_ATTRIBUTES("add.balance.to.extended.attributes",Constants.FALSE, "If true, it will add the scheme balance into the scheme calculation extended attributes"),
	EC_RATE_APPLICABLE("ec.rate.applicable",Constants.FALSE ,"If true, it will apply ec rate type schemes" ),
	SET_KG_QUANTITY("set.kg.quantity",Constants.FALSE ,"If true , it will set the total kg quantity for the given batch code" ),
	GIVE_PARTIAL_DISCOUNT("give.partial.discount",Constants.FALSE ,"If true, it will give partial discount when available balance is less than discount" ),
	ADD_GLOBAL_PRODUCTS_IN_PRICING_RESPONSE("add.global.products.in.pricing.response",Constants.FALSE,"If true, it will check in product metadadta for global pricing common for all outlets i.e. supplier and outletcode both having a null value"),
	USE_PRODUCT_METADATA_OTHER_UNIT_MRP("use.productmetadata.other.unit.mrp", Constants.FALSE,
			"If true then otherUnitMrp will used from the product metadata"),
	PRICING_FILTER("pricing.filter",Constants.FALSE,"if true, it will filter product pricing on the basis of supplier id"),
	USE_OUTLETCODE_FOR_ORDER_GET_FOR_RETAILER("use.outletcode.for.order.get.retailer",Constants.FALSE,"If true outletCode will be used to fetch orders for retailers"),
	USE_OUTLETCODE_FOR_SALES_GET_FOR_RETAILER("use.outletcode.for.sales.get.retailer",Constants.FALSE,"If true outletCode will be used to fetch sales for retailers"),
	REDIRECT_TO_OUTLET_PRICING_ADJUSTMENT("redirect.to.outlet.pricing.adjustment",Constants.FALSE, "IF true will redirect the prcing condition flow to outlet pricing adjustment flow"),
	USE_NEW_NOTIFICATION_SERVICE("use.new.notification.service",Constants.FALSE,"If true, it will use the new notification service");
