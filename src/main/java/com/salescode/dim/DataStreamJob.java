...
//                .name("Aggregate Window Count")
        ...
        ConfigValidator.validate(outputProperties, "lob", "topic");

		String lobTopicName = outputProperties.getProperty("lob") + outputProperties.getProperty("topic");

		KafkaRecordSerializationSchema<StreamingRawData> recordSerializationSchema = KafkaRecordSerializationSchema.<StreamingRawData>builder()
				.setTopic(lobTopicName)
				// Use a field as kafka record key
				// Define no keySerializationSchema to publish kafka records with no key
				.setKeySerializationSchema(srd -> srd.getRequestId().getBytes())
				// Serialize the Kafka record value (payload) as JSON
				.setValueSerializationSchema(new JsonSerializationSchema<>())
				.build();

		// Create and add the Sink
		KafkaSink<StreamingRawData> sink = FlinkJobSink.createKafkaSink(outputProperties, recordSerializationSchema);
		input.sinkTo(sink);
