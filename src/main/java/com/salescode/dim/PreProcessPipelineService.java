...
public class PreProcessPipelineService implements Serializable {
    ...
    Logger logger = LoggerFactory.getLogger(PreProcessPipelineService.class);
    ...
    public PreProcessOperationResult preProcessPipeline(CommonDataModel commonDataModel, String preprocessValidationExcludeGroup) {
        ...
        if (finalResult.getPreValidationEnrichment().getStatus().equals(OperationResult.Status.OK)) {
            OperationResult validate = dataValidationService.validate(finalResult.getPreValidationEnrichment()
                    .getOperationResultData(), preprocessValidationExcludeGroup);
        ...
        if (finalResult.getValidation().getStatus().equals(OperationResult.Status.OK)) {
            OperationResult postValidationEnrich = dataEnrichmentService.enrich(finalResult.getValidation()
                    .getOperationResultData(), EnrichmentPhase.POST_VALIDATION);
            finalResult.setPostValidationEnrichment(postValidationEnrich);
        }
        long pstart3 = System.currentTimeMillis();

        if (finalResult.getPreValidationEnrichment().getStatus()
                .equals(OperationResult.Status.OK) && finalResult.getValidation().getStatus()
                .equals(OperationResult.Status.OK) && finalResult.getPostValidationEnrichment().getStatus()
                .equals(OperationResult.Status.OK)) {
    ...
    public void evaluateFailures(PreProcessOperationResult response, List<String> errorList) {
        if (response.getPreValidationEnrichment() != null && response.getPreValidationEnrichment().getStatus()
                .equals(OperationResult.Status.ERROR)) {
            List<OperationResult.StepResult> enrichmentResult = response.getPreValidationEnrichment().getStepResults();
            String error = StringUtils.join(enrichmentResult.stream().map(EnrichmentResult::getMessage)
                    .collect(Collectors.toList()), SEPARATOR);
            errorList.add(ENRICHMENT_ERROR + error);
        }
        if (response.getValidation() != null && response.getValidation().getStatus()
                .equals(OperationResult.Status.ERROR)) {
            List<OperationResult.StepResult> ruleresult = response.getValidation().getStepResults();
            String error = StringUtils.join(ruleresult.stream().map(EnrichmentResult::getMessage)
                    .collect(Collectors.toList()), SEPARATOR);
            errorList.add(VALIDATION_ERROR + error);
        }
        if (response.getPostValidationEnrichment() != null && response.getPostValidationEnrichment().getStatus()
                .equals(OperationResult.Status.ERROR)) {
