...
public class EntityValidationService {
	...
	private EntityValidationResult evaluateResults(List<RuleResult> ruleResult) {
		...
		if(!ObjectUtils.isEmpty(errorresult)) {
			vr.setMessage(org.apache.commons.lang3.StringUtils.join(
					errorresult.stream()
					.filter(p->StringUtils.isNotEmpty(p.getMessage()))
					.map(RuleResult::getMessage).collect(Collectors.toList()),",")
			);
		}
	...
	@Autowired
	private FormValidator entityValidation;
