...
import com.salescode.channelkart.models.diff.ChangeType;
...
public class CdmDiffUtil {
    ...
    public static <T extends CommonDataModel> T withOldModel(Supplier<T> supplier) {
        T model = supplier.get();
        if (model == null) {
            return null;
        }
        T o = EntityUtils.deepClone(model);
        model.setOldModel(o);
        return model;
    }

    public static <T extends CommonDataModel> T setOldModel(T model) {
        if (model != null) {
            model.setOldModel(EntityUtils.deepClone(model));
        }
        return model;
    }
