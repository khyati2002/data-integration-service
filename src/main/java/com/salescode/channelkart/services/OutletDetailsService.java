...
public class OutletDetailsService extends AbstractCDMService<CkOutletDetails> {
    ...
    @Autowired
    public OutletDetailsService(DSLContext dsl, HierarchyMetaDataToStringConverter hierarchyMetaDataToStringConverter, UserService userService, SupplierInfoService supplierInfoService, RoleService roleService, MetadataRegistry metadataRegistry, HierarchyMetaDataService hierarchyMetaDataService) {
    ...
    public CkLocation getLocation(CkOutletDetails outlet) {
        ...
        return dsl.select(CK_OUTLET_DETAILS.fields()).from(CK_OUTLET_DETAILS).join(CK_LOCATION).on(CK_OUTLET_DETAILS.LOCATION_HIERARCHY.eq(CK_LOCATION.LOCATION_HIERARCHY)).where(CK_OUTLET_DETAILS.ID.eq(outlet.getId())).fetchOneInto(CkLocation.class);
    }
...
//            outletDetails.setLocationHierarchy(location);
    ...
    public CkUser getUserName(CkOutletDetails outlet) {
        return dsl.select(CK_OUTLET_DETAILS.fields()).from(CK_OUTLET_DETAILS).join(CK_USER).on(CK_OUTLET_DETAILS.LOGINID.eq(CK_USER.LOGINID)).where(CK_OUTLET_DETAILS.ID.eq(outlet.getId())).fetchOneInto(CkUser.class);
    }

    public String getLoginId(CkUser user) {
        return dsl.select(CK_USER.ID).from(CK_USER).where(CK_USER.ID.eq(user.getId())).fetchOneInto(String.class);
    ...
    public List<CkHierarchyMetadata> getImmediateParent(CkOutletDetails outlet) {
        return dsl.select(CK_HIERARCHY_METADATA.fields())  // Select fields from the HierarchyMetaData table
                .from(CK_OUTLET_DETAILS).join(CK_OUTLET_DETAILS_HIERARCHYMETADATA).on(CK_OUTLET_DETAILS.ID.eq(CK_OUTLET_DETAILS_HIERARCHYMETADATA.OUTLET_ID))  // Join on outlet_id
                .join(CK_HIERARCHY_METADATA).on(CK_OUTLET_DETAILS_HIERARCHYMETADATA.HIERARCHY_METADATA_ID.eq(CK_HIERARCHY_METADATA.ID))  // Join on hierarchy_metadata_id
...
//               .fetchSet(CK_USERDESIGNATION.DESIGNATION);
    ...
    private List<CkHierarchyMetadata> getImmediateParent(CkUser user) {
        var result = dsl.select(CK_USER_PARENT.PARENT).from(CK_USER_PARENT).where(CK_USER_PARENT.USERLOGINID.eq(user.getId())).fetch();
...
//                    k -> userService.findByLoginId(user.getLoginId(), false));
    ...
    private CkUser updateUser(CkUser user) {
        ...
        if (user.canHash() && StringUtils.isNotEmpty(existingHash) && existingHash.equals(user.getHash()) && user.getChanges().isEmpty()) {
            return user;
        } else {
...
//    //
    ...
    private void setHierarchy(CkOutletDetails tempoutlet) {
        ...
        if (immediateParent != null && !immediateParent.isEmpty()) {
            String hierarchy = String.join(",", list.stream().map(s -> s.getHierarchy()).collect(Collectors.toList()));
            if (StringUtils.isNotEmpty(hierarchy)) {
