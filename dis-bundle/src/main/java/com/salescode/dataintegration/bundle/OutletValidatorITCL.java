...
public class OutletValidatorITCL extends AbstractValidationRule<CkOutletDetails> {
    ...
    public RuleResult apply(CkOutletDetails cdm) {
        ...
        try {
            ...
            CkLocation location = cdm.getLocation();
            if(location != null) {
            ...
            if (cdm.getLocation().getDistrict() != null) {
                if (!regexValidation.match(capitalCaseRegex, cdm.getLocation().getDistrict())) {
                    ruleResult.add(
                            "Value given for district should contain only alphabets with capital case.Current given value is not compatible");
                }
            } else {
